...
class HOMRDataset:
    ...
    train_n, dev_n, test_n = 51365, 5027, 5023
    ...
    def __init__(self) -> None:
        for dataset, size in [("train", HOMRDataset.train_n), ("dev", HOMRDataset.dev_n), ("test", HOMRDataset.test_n)]:
            path = "homr.{}.tfrecord".format(dataset)
            if not os.path.exists(path):
                print("Downloading file {}...".format(path), file=sys.stderr)
                urllib.request.urlretrieve("{}/{}".format(self._URL, path), filename=path + ".tmp")
                os.rename(path + ".tmp", path)
            setattr(self, dataset + "_n", size)
    ...
    class EditDistanceMetric(tf.metrics.Mean):
        ...
        def update_state(self, y_true: tf.RaggedTensor, y_pred: tf.RaggedTensor,
